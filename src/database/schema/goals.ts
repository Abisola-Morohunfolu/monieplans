...
export const savingsGoals = sqliteTable('savings_goals', {
  ...
  currentSavedAmountCents: integer('current_saved_amount_cents')
    .notNull()
    .default(0),
  targetDate: text('target_date'),
  priorityRank: integer('priority_rank').notNull().default(0),
  status: text('status').notNull().default('active'),
  reserveInBudget: integer('reserve_in_budget', { mode: 'boolean' })
    .notNull()
    .default(false),
  notes: text('notes'),
...
export const goalBudgetReservations = sqliteTable(
  ...
  (t) => [
    unique('uq_goal_reservations_period_goal').on(t.budgetPeriodId, t.goalId),
  ],
);
