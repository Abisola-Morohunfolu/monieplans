...
import { createGoalSchema, updateGoalSchema, reserveGoalSchema } from '../shared/schemas';
...
goalsRouter.post('/', validateJson(createGoalSchema), async (c) => {
  ...
  const db = c.get('db');
  const body = c.get('body') as unknown as ReturnType<typeof createGoalSchema.parse>;
...
goalsRouter.get('/:id', async (c) => {
  ...
  const [goal] = await db
    .select()
    .from(schema.savingsGoals)
    .where(and(eq(schema.savingsGoals.userId, user.id), eq(schema.savingsGoals.id, id)));

  if (!goal) throw new HTTPException(404, { message: 'Savings goal not found' });
  return c.json(goal);
});
...
goalsRouter.patch('/:id', validateJson(updateGoalSchema), async (c) => {
  ...
  const db = c.get('db');
  const id = c.req.param('id')!;
  const body = c.get('body') as unknown as ReturnType<typeof updateGoalSchema.parse>;
  ...
  if (body.targetAmount !== undefined) setValues.targetAmountCents = toCents(body.targetAmount);
  if (body.targetDate !== undefined) setValues.targetDate = body.targetDate;
  if (body.priorityRank !== undefined) setValues.priorityRank = body.priorityRank;
  if (body.reserveInBudget !== undefined) setValues.reserveInBudget = body.reserveInBudget;
...
goalsRouter.post('/reserve/:budgetPeriodId', validateJson(reserveGoalSchema), async (c) => {
  const user = c.get('user');
  const db = c.get('db');
  const budgetPeriodId = c.req.param('budgetPeriodId')!;
  ...
  return db.transaction(async (tx) => {
    const reservations = [];
    const body = c.get('body') as unknown as ReturnType<typeof reserveGoalSchema.parse>;
