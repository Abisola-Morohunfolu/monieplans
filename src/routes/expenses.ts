...
import { createExpenseSchema, updateExpenseSchema, listExpensesQuerySchema } from '../shared/schemas';
...
expensesRouter.post('/', validateJson(createExpenseSchema), async (c) => {
  ...
  const db = c.get('db');
  const body = c.get('body') as unknown as ReturnType<typeof createExpenseSchema.parse>;
  ...
  return db.transaction(async (tx) => {
    ...
    if (!budgetPeriod) {
      throw new HTTPException(400, { message: 'No active budget period found. Please activate a budget period first.' });
    }
      ...
      const [cat] = await tx
        ...
        .where(
          and(
            ...
            or(isNull(schema.categories.userId), eq(schema.categories.userId, user.id)),
          ),
        );
...
expensesRouter.get('/', validateQuery(listExpensesQuerySchema), async (c) => {
  const user = c.get('user');
  const db = c.get('db');
  const query = c.get('query') as unknown as ReturnType<typeof listExpensesQuerySchema.parse>;

  const conditions: ReturnType<typeof eq>[] = [
    eq(schema.expenseEntries.userId, user.id),
    isNull(schema.expenseEntries.deletedAt) as ReturnType<typeof eq>,
  ];

  if (query.budgetPeriodId) {
    conditions.push(eq(schema.expenseEntries.budgetPeriodId, query.budgetPeriodId));
  }
  if (query.weeklyBudgetAllocationId) {
    conditions.push(eq(schema.expenseEntries.weeklyBudgetAllocationId, query.weeklyBudgetAllocationId));
  }
  if (query.categoryId) {
  ...
  if (query.search) {
    ...
    conditions.push(
      sql`(LOWER(${schema.expenseEntries.description}) LIKE LOWER(${searchTerm}) OR LOWER(${schema.expenseEntries.merchantName}) LIKE LOWER(${searchTerm}))` as ReturnType<typeof eq>,
    );
  }
...
expensesRouter.patch('/:id', validateJson(updateExpenseSchema), async (c) => {
  ...
  const db = c.get('db');
  const id = c.req.param('id')!;
  const body = c.get('body') as unknown as ReturnType<typeof updateExpenseSchema.parse>;

  const [expense] = await db
    .select()
    .from(schema.expenseEntries)
    .where(and(eq(schema.expenseEntries.id, id), eq(schema.expenseEntries.userId, user.id), isNull(schema.expenseEntries.deletedAt)));
  ...
  return db.transaction(async (tx) => {
    ...
    if (body.categoryId !== undefined) {
      if (body.categoryId) {
        const [cat] = await tx
          .select()
          .from(schema.categories)
          .where(and(eq(schema.categories.id, body.categoryId!), or(isNull(schema.categories.userId), eq(schema.categories.userId, user.id))));
        if (!cat) throw new HTTPException(404, { message: 'Category not found' });
      }
    ...
    if (newWeekId && newWeekId !== oldWeekId) await recalculateWeeklyCache(tx, newWeekId);
