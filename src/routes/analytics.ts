...
import { updateRecommendationStatusSchema, generateInsightsSchema } from '../shared/schemas';
...
analyticsRouter.patch('/recommendations/:id/status', validateJson(updateRecommendationStatusSchema), async (c) => {
  const user = c.get('user');
  const db = c.get('db');
  const id = c.req.param('id')!;
  const { status } = c.get('body') as unknown as { status: 'dismissed' | 'accepted' };
  ...
  if (!snapshot) throw new HTTPException(404, { message: 'Recommendation not found' });
  return c.json(snapshot);
});

analyticsRouter.post('/generate-insights', validateJson(generateInsightsSchema), async (c) => {
  const user = c.get('user');
  const db = c.get('db');
  const { budgetPeriodId } = c.get('body') as unknown as { budgetPeriodId: string };
