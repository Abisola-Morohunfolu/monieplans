...
import { serializeIncome } from '../shared/serializers';
import {
  createIncomeSchema,
  updateIncomeSchema,
...
incomeRouter.post('/', validateJson(createIncomeSchema), async (c) => {
  ...
  return c.json(serializeIncome(income), 201);
});
...
incomeRouter.get('/', validateQuery(listIncomeQuerySchema), async (c) => {
  ...
  return c.json(paginated(income.map(serializeIncome), total, limit, offset));
});

incomeRouter.patch('/:id', validateJson(updateIncomeSchema), async (c) => {
  const user = c.get('user');
  const db = c.get('db');
  const id = c.req.param('id')!;
  const body = c.get('body') as unknown as ReturnType<
    typeof updateIncomeSchema.parse
  >;
  ...
  return c.json(serializeIncome(updated));
});
